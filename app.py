...
def api_ton_deposit_address():
    ...
    try:
        ...
        with get_cursor() as cur:
            cur.execute(
                "SELECT deposit_id FROM ton_deposits WHERE user_id=%s ORDER BY created_at DESC LIMIT 1",
                (str(user_id),)
            )
            row = cur.fetchone()
            deposit_id = (row['deposit_id'] if isinstance(row, dict) else row[0]) if row else None
    except Exception as e:
...
def api_ton_deposit_status(deposit_id):
    ...
    if not row:
        # Also check if any new deposit has been credited since polling started
        try:
            from database import get_cursor
            with get_cursor() as cur:
                cur.execute(
                    "SELECT deposit_id, amount, status, tx_hash FROM ton_deposits WHERE user_id=%s AND status='confirmed' ORDER BY credited_at DESC LIMIT 1",
                    (str(user_id),)
                )
                row = cur.fetchone()
        except Exception:
