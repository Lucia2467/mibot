...
def api_arcade_claim_minute():
    ...
    try:
        ...
        # Verificar minutos jugados hoy — usar alias para cursor dict
        with get_cursor() as cursor:
            cursor.execute("""
                SELECT COALESCE(SUM(minutes_played), 0) AS mins
                FROM arcade_sessions
...
def api_vpn_check():
    """VPN/Proxy check endpoint - used by vpn_detector.js frontend"""
    try:
        if VPN_SYSTEM_AVAILABLE:
            from vpn_system import is_vpn_or_proxy
            client_ip = get_client_ip()
            is_vpn, reason = is_vpn_or_proxy(client_ip)
            return jsonify({'vpn_detected': is_vpn, 'reason': reason})
        else:
            # VPN system not available - allow all traffic
            return jsonify({'vpn_detected': False, 'reason': 'vpn_system_unavailable'})
    except Exception as e:
        logger.error(f"[api_vpn_check] Error: {e}")
        return jsonify({'vpn_detected': False, 'error': True})

@app.route('/api/db-status', methods=['GET'])
def api_db_status():
    """Database connection status - used by db_loader.js frontend"""
    try:
        from db import execute_query
        execute_query("SELECT 1")
        return jsonify({'status': 'connected', 'is_connecting': False, 'message': '✅ Connected'})
    except Exception as e:
        return jsonify({'status': 'connecting', 'is_connecting': True, 'message': '⏳ Connecting...'})

@app.route('/vpn-blocked')
def vpn_blocked():
    """VPN blocked page"""
    user_id = get_user_id()
...
def api_ton_deposit_address():
    ...
    memo = 'DEP' + digits[-8:].zfill(8) if digits else 'DEP00000001'

    ton_wallet = os.environ.get('TON_WALLET_ADDRESS', 'UQD0vWmw4lH9O8UTPrU2ZLmvlRfbNJOilQQMgmDSQh8X6gH3')

    # Get or create a pending deposit_id for polling
    # Always returns an ID so the frontend polling can start immediately
    try:
        ...
        with get_cursor() as cur:
            # Prefer a confirmed deposit, then any pending one
            cur.execute(
                """SELECT deposit_id FROM ton_deposits
                   WHERE user_id=%s
                   ORDER BY (status='confirmed') DESC, credited_at DESC
                   LIMIT 1""",
                (str(user_id),)
            )
            row = cur.fetchone()
            if row:
                deposit_id = row['deposit_id'] if isinstance(row, dict) else row[0]

        # No record at all - create a pending placeholder so polling has an ID
        if not deposit_id:
            deposit_id = f"PENDING_{user_id}_{int(_time.time())}"
            try:
                execute_query(
                    """INSERT IGNORE INTO ton_deposits
                       (deposit_id, user_id, wallet_destination, amount, status, updated_at)
                       VALUES (%s, %s, %s, 0, 'pending', NOW())""",
                    (deposit_id, str(user_id), ton_wallet)
                )
            except Exception:
                pass  # table may lack some columns, polling will still work via status endpoint fallback

    except Exception as e:
        logger.error(f"[ton_deposit_address] DB error: {e}")
        deposit_id = f"PENDING_{user_id}"
...
@app.route('/api/ton/deposit/status/<deposit_id>', methods=['GET'])
def api_ton_deposit_status(deposit_id):
    """Polls the status of a specific TON deposit"""
    user_id = get_user_id()
    if not user_id:
        return jsonify({'success': False, 'error': 'User ID required'}), 400
    ...
    try:
        ...
        with get_cursor() as cur:
            cur.execute(
                "SELECT deposit_id, user_id, amount, status, tx_hash, credited_at FROM ton_deposits WHERE deposit_id=%s AND user_id=%s",
                (deposit_id, str(user_id))
            )
            row = cur.fetchone()
    except Exception as e:
        logger.error(f"[ton_deposit_status] DB error: {e}")
        return jsonify({'success': False, 'error': 'DB error'}), 500

    if not row:
        # deposit_id not found directly - could be a PENDING_ placeholder or stale id.
        # Fall back to checking for any confirmed deposit for this user.
        try:
            from database import get_cursor
            with get_cursor() as cur:
                cur.execute(
                    """SELECT deposit_id, amount, status, tx_hash
                       FROM ton_deposits
    ...
    if not row:
        return jsonify({'success': True, 'status': 'pending', 'ton_amount': 0})
...
@app.route('/api/ton/deposit/history', methods=['GET'])
def api_ton_deposit_history():
    """Returns TON deposit history for the user"""
    user_id = get_user_id()
    if not user_id:
        return jsonify({'success': False, 'error': 'User ID required'}), 400
    ...
    try:
        ...
        with get_cursor() as cur:
            cur.execute(
                """SELECT deposit_id, amount, status, tx_hash, credited_at as created_at
        ...
        logger.error(f"[ton_deposit_history] DB error: {e}")
        return jsonify({'success': False, 'deposits': []}), 500
    ...
    for r in rows:
        if isinstance(r, dict):
            deposits.append({
                ...
                'ton_amount': float(r.get('amount', 0)),
            ...
            deposits.append({
                'deposit_id': r[0],
                'ton_amount': float(r[1]) if r[1] else 0,
