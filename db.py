...
Optimized for PythonAnywhere $5 plan (limited resources)
FIXED VERSION - Compatible with PythonAnywhere MySQL 8
"""
...
if not _mysql_password:
    logger.warning("⚠️ WARNING: MYSQL_PASSWORD environment variable is missing or empty!")
    logger.warning("⚠️ Database connections will fail without a password.")
    logger.warning("⚠️ Please set MYSQL_PASSWORD in your .env file.")
...
DB_CONFIG = {
    'host': os.getenv('MYSQL_HOST', 'Isaax23.mysql.pythonanywhere-services.com'),
    'database': os.getenv('MYSQL_DATABASE', 'Isaax23$sally_db'),
    'user': os.getenv('MYSQL_USER', 'Isaax23'),
    ...
    'buffered': True,
    # NOTE: 'use_pure' removed - not supported on PythonAnywhere
}

# Pool settings - OPTIMIZED FOR PYTHONANYWHERE $5 PLAN
POOL_SIZE = 3  # Small pool size to avoid resource exhaustion
...
def _get_friendly_error_message(error):
    ...
    if 'can\'t connect' in error_str or 'connection refused' in error_str or error_code == 2003:
        return "❌ Cannot connect to database server. Check MYSQL_HOST is correct."

    # Unknown database
    if 'unknown database' in error_str or error_code == 1049:
        return "❌ Database does not exist. Check MYSQL_DATABASE is correct."

    # Access denied / wrong credentials
    if 'access denied' in error_str or error_code == 1045:
        return "❌ Access denied. Check MYSQL_USER and MYSQL_PASSWORD are correct."
...
def _create_direct_connection():
    ...
    if not _mysql_password:
        raise MySQLError("Cannot connect: MYSQL_PASSWORD is not set")
...
class DatabasePool:
    ...
    _pool = None
    _initialized = False
    _pool_failed = False  # Track if pool creation has failed
    ...
    def _create_pool(self):
        ...
        if not _mysql_password:
            logger.error("❌ Cannot create pool: MYSQL_PASSWORD is not set")
            self._pool = None
            self._pool_failed = True
            return

        try:
            # Create clean config for pool (remove unsupported params)
            pool_config = {
                'host': DB_CONFIG['host'],
                'database': DB_CONFIG['database'],
                'user': DB_CONFIG['user'],
    ...
    def get_connection(self):
        ...
        if not _mysql_password:
            raise MySQLError("Cannot connect: MYSQL_PASSWORD environment variable is not set. Please configure your .env file.")
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                # Pool not available - try to create it
                elif not self._pool_failed:
                    self._create_pool()
                    if self._pool is not None:
                        continue  # Retry with new pool

                # Fallback to direct connection (no pooling)
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    # On pool exhaustion, try to recreate pool
        ...
        # All attempts failed - raise with friendly message
        if last_error:
...
def execute_query(query, params=None):
    """
    Execute a query - PythonAnywhere compatible version.

    IMPORTANT: This function does NOT support fetch parameters.
    Use get_cursor() with fetchone()/fetchall() for SELECT queries.

    This function is intended for INSERT/UPDATE/DELETE operations.
...
def test_connection():
    ...
    try:
        ...
        if not _mysql_password:
            logger.error("❌ Database test failed: MYSQL_PASSWORD is not set")
            return False
